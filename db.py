...
def conectar():
    try:
        ...
        print('Error en la conexión con la Base de dartos. Cód. Error: ' +  str(err))
        return None

def insert_prov(_sql, lista_parametros):
    try:
        conn = conectar()
        ...
        if conn:
            ...
            print("No se pudo establecer la conexión. Favor revisar errores desde la conexión previa")
            return -1
    except Error as err:
...
def prov_select(_sql, lista_parametros):
    try:
        conn = conectar()
        if conn:
...
def insert_usu(_sql, lista_parametros):
    try:
        conn = conectar()
...
def usu_select(_sql, lista_parametros):
    try:
        conn = conectar()
        if conn:
